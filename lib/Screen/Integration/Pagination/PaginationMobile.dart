...
class PaginationStateMobile extends State<PaginationMobile> {
  ...
  _scrollListener() {
    if (controller.offset >= controller.position.maxScrollExtent &&
      ...
      if (this.mounted) {
        setState(() {
          if (perPageLimit < itemList.length) loadMore();
        });
      }
    }
  }
  ...
  void loadMore() {
    setState(() {
      if ((offset + perPageLimit) > itemList.length) {
        tempList.addAll(itemList.getRange(offset, itemList.length));
      } else {
        tempList.addAll(itemList.getRange(offset, offset + perPageLimit));
      }
      offset = offset + perPageLimit;
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        centerTitle: true,
        elevation: 0.0,
        title: new Text("Pagination", style: TextStyle(color: Colors.black)),
      ...
      body: ListView.builder(
        ...
        itemCount:
            (offset < itemList.length) ? tempList.length + 1 : tempList.length,
